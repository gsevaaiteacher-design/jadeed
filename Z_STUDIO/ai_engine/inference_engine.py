...
# System ki saari pol kholne wala code
print(f"--- [SYSTEM_DEBUG_DUMP] ---")
print(f"Current File: {__file__}")
print(f"Python Path: {sys.path}")
print(f"Loaded Modules:")
for name, module in sys.modules.items():
    if "inference_engine" in name:
        print(f"Found Inference Module at: {getattr(module, '__file__', 'UNKNOWN')}")
print(f"---------------------------")
...
print(f"!!! LOADING INFERENCE ENGINE FROM: {__file__} !!!")

def cctv(func):
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        print(f"\n>>> [CCTV_ENTRY] Function: {func.__name__}")
        start_time = time.time()
        try:
            result = func(*args, **kwargs)
            duration = time.time() - start_time
            print(f">>> [CCTV_EXIT] Function: {func.__name__} | Time: {duration:.4f}s")
            return result
        except Exception as e:
            print(f">>> [CCTV_CRASH] Function: {func.__name__} | ERROR: {str(e)}")
            raise e
    return wrapper
...
class InferenceEngine:
    ...
    def __init__(self, model_path=None, model_type="UNIVERSAL", backend="AUTO", runtime_bridge=None, runtime_bus=None):
        ...
        print(f"[DEBUG_BUS] Is Bus Real? {self._bus_ready} | ID: {id(self.bus)}")

        # 3. INDUSTRIAL SUBSCRIPTION (Moved to Hard-Wiring)
        # 4. AI CORE HOOKS
        ...
        # 6. BUS SIGNAL CONTRACT & DIAGNOSTICS
        # (Diagnostics removed to prevent interference)
        ...
        print(f"[DEBUG] INFERENCE BUS ID: {id(self.bus)}")

    def _apply_hard_wiring(self):
        """State Manager ke 'Clean-up' ke baad force subscription."""
        if not getattr(self, "_bus_ready", False): return print("[BUS] Not ready yet - skipping wiring")
        if not self.bus: return print("[BUS_ERROR] Bus not attached")
        try:
            self.bus.subscribe("inference", self._bus_inference_entry)
            self.bus.subscribe("ENGINE_PING", self._handle_ping)
            print("[FINAL_FIX] Subscription applied AFTER State Sync. Persistence established.")
        except Exception as e: print(f"[WIRING_CRASH] {e}")

    @cctv
    def _trace_entry(self, task):
        """Debug trace entry point"""
        print(f"[TRACE] REQUEST REACHED ENGINE: {task}")
    ...
    def bind_model_handle(self, handle):
        ...
        print(f"[DEBUG] Model handle successfully injected into InferenceEngine.")
    @cctv
    def initialize_db(self):
        """[NEW] Late-binding for Vector System to ensure Bus is ready."""
        try:
            from data_core.vector_db import VectorDB
            self.vector_core = VectorCore()
            self.vector_db = VectorDB(runtime_bus=self.bus)
            
            # Re-bind embedder if needed
    ...
    def execute_inference(self, payload):
        ...
        print(f" [INFERENCE_RECEIVED]: {trace_id}")

        # FIX 2: Prompt normalization
        ...
        if getattr(self, "safety_layer", None):
            # Prompt string ho ya dict, safety layer ko wahi bhejo jo validate ho sake
            # Agar model_path nahi hai toh payload se lo
        ...
        if not model_handle or model_handle == "ZAI_PRIMARY_CORE":
            ...
            if getattr(self, "bus", None): self.bus.publish("EXECUTION_DONE", resp)
            return resp

        # SCHEDULER
        if getattr(self, "scheduler", None):
            try: self.scheduler.queue_task(payload)
            except Exception as e: print(f"[SCHEDULER_WARN]: {e}")

        # EXECUTION LAYER
        try: result = self.execute(model_handle, payload)
        except Exception as e: return self._fail("EXECUTION_CRASH", str(e), trace_id)
        ...
        if is_success:
            ...
            if getattr(self, "bus", None): self.bus.publish("EXECUTION_DONE", final_response)
    ...
    def execute(self, model_handle, payload):
        ...
        # FIX BUG #3: Bridge Availability Check
        ...
        print(f" [ENGINE] Payload processing: {trace_id}")
    ...
    def _normalize(self, raw_data, media_type, attempts, model_handle):
        ...
        # FIX: Agar raw_data dict hai, toh uska 'content' ya 'data' nikalo
        content = raw_data
    ...
    def get_active_channel(self, channel_type="default"):
        # Yahan return karo jo tumhara system expect kar raha hai
        # Agar tumhare system mein koi channels hain, toh unhe yahan se handle karo
...
if __name__ == "__main__":
    ...
    # IMPORTANT: whichever method exists
